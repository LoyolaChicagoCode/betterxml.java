...
public class FirstTest extends TestCase {
	...
	public void testValidateArgs() {
		...
		try {
			...
			assertTrue(ifa.getMessage().contains("First takes no arguments."));
		}
	...
	public void testEvaluate() {
		...
		List<XNode> results = first.evaluate(currentResults());
		...
		first = null;
		try {
			first = new First(null);
		} catch (InvalidFunctionArguments ifa) {
			fail("An InvalidFunctionArguments exception should not have been thrown!");
		}
		
		results = first.evaluate(currentResults());
		assertEquals(1, results.size());
		assertTrue(results.get(0) instanceof XNode);
