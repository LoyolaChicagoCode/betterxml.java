...
public class HasAttributeWithValueContainsTest extends TestCase {
	...
	public void testEvaluate() {
		String[] args = new String[]{"number", "ne"};
		HasAttributeWithValueContains hawvc = null;
		try {
			hawvc = new HasAttributeWithValueContains(args);
		} catch (InvalidFunctionArguments ifa) {
			fail("An InvalidFunctionArguments exception should not have been thrown!");
		}
		
		List<XNode> results = hawvc.evaluate(currentResults());
		...
		results = hawvc.evaluate(currentResults());
